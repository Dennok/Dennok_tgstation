...
///The shuttle datum, all info about shuttle stored here 
/datum/shuttle
	var/name = "Shuttle" 	///The name shown to users.
	var/id					///Unical identifier to work with system. If we have more than one shuttles with one ID work only one
	
	dir = EAST		///The dir of our shuttle. Almost all started from EAST
	...
	var/x	///cords of our dock marker, while i dont change it
	var/y

///returns a list(x0,y0, x1,y1) where points 0 and 1 are bounding corners of the projected rectangle
/datum/shuttle/proc/return_coords(_x, _y, _dir)
	if(_dir == null)
		_dir = dir
	if(_x == null)
		_x = x
...
/datum/shuttle/proc/return_turfs()
	var/list/L = return_coords()
	var/turf/T0 = locate(L[1],L[2],z)
	var/turf/T1 = locate(L[3],L[4],z)
	return block(T0,T1)

/datum/shuttle/proc/return_ordered_turfs(_x, _y, _z, _dir)
	var/cos = 1
	var/sin = 0
	switch(_dir)
		if(WEST)
...
/datum/shuttle/proc/highlight(_color)
	var/list/L = return_coords()
	var/turf/T0 = locate(L[1],L[2],z)
	var/turf/T1 = locate(L[3],L[4],z)
...
/datum/shuttle/proc/get_docked()
	return

/datum/shuttle/proc/getDockedId()
	return //P.id

/datum/shuttle/proc/is_in_shuttle_bounds(atom/A)
...
/datum/shuttle
	...
	var/list/shuttle_areas			//list of areas what we rules
...
/datum/shuttle/proc/register()
	if(SSshuttle.shuttle[id])
		WARNING("[src] [id] already in SSshuttle.mobile! Replacing existing id.")
	if(!id || id == initial(id))
		WARNING("[src] dont have unique id on register(). Generatng.")
		id = "[id][GUID()]"
	SSshuttle.shuttle[id] = src

/datum/shuttle/Destroy(force)
	if(force)
		SSshuttle.shuttle -= id
		destination = null
		previous = null
...
/datum/shuttle/Initialize(mapload)
	. = ..()

	SSshuttle.shuttle_counter++
	id = "[destination_type]_[SSshuttle.shuttle_counter]"
	name = "[name] shuttle[SSshuttle.shuttle_counter]"
...
/datum/shuttle/proc/linkup(datum/map_template/shuttle/template, obj/docking_port/stationary/dock)
...
/datum/shuttle/proc/canMove()
	return TRUE

//this is to check if this shuttle can physically dock at dock S
/datum/shuttle/proc/canDock(obj/docking_port/stationary/S)
	if(!istype(S))
...
/datum/shuttle/proc/check_dock(obj/docking_port/stationary/S, silent=FALSE)
...
/datum/shuttle/proc/request(obj/docking_port/stationary/S)
	if(!check_dock(S))
...
/datum/shuttle/proc/enterTransit()
...
/datum/shuttle/proc/jumpToNullSpace()
...
/datum/shuttle/proc/intoTheSunset()
	// Loop over mobs
...
/datum/shuttle/proc/create_ripples(obj/docking_port/stationary/S1, animate_time)
	var/list/turfs = ripple_area(S1)
	for(var/t in turfs)
		ripples += new /obj/effect/abstract/ripple(t, animate_time)

/datum/shuttle/proc/remove_ripples()
	QDEL_LIST(ripples)

/datum/shuttle/proc/ripple_area(obj/docking_port/stationary/S1)
...
/datum/shuttle/proc/check_effects()
	if(!ripples.len)
...
/datum/shuttle/proc/parallax_slowdown()
	for(var/place in shuttle_areas)
		var/area/shuttle/shuttle_area = place
...
/datum/shuttle/proc/setTimer(wait)
	timer = world.time + wait
	last_timer_length = wait

/datum/shuttle/proc/modTimer(multiple)
...
/datum/shuttle/proc/invertTimer()
...
//returns timeLeft
/datum/shuttle/proc/timeLeft(divisor)
	if(divisor <= 0)
		divisor = 10
...
/datum/shuttle/proc/getTimerStr()
...
/datum/shuttle/proc/getStatusText()
...
/datum/shuttle/proc/getDbgStatusText()
	var/obj/docking_port/stationary/dockedAt = get_docked()
...
/datum/shuttle/proc/getControlConsole()
	for(var/place in shuttle_areas)
		var/area/shuttle/shuttle_area = place
...
/datum/shuttle/proc/hyperspace_sound(phase, list/areas)
	var/selected_sound
	switch(phase)
...
/datum/shuttle/proc/alter_engines(mod)
	if(mod == 0)
		return
...
/datum/shuttle/proc/count_engines()
	. = 0
...
/datum/shuttle/proc/get_engine_coeff(current,engine_mod)
...
/datum/shuttle/emergency/in_flight()
	switch(mode)
		if(SHUTTLE_ESCAPE)
			return TRUE
...
/datum/shuttle/proc/on_emergency_launch()
...
/datum/shuttle/emergency/on_emergency_launch()
	return

//Called when emergency shuttle docks at centcom
/datum/shuttle/proc/on_emergency_dock()
...
/datum/shuttle/pod/on_emergency_dock()
	if(launch_status == ENDGAME_LAUNCHED)
		initiate_docking(SSshuttle.getDock("[id]_away")) //Escape pods dock at centcom
		mode = SHUTTLE_ENDGAME

/datum/shuttle/emergency/on_emergency_dock()
	return
