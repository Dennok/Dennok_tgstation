...
/obj/item/assembly/control/tram
	...
	var/to_where_id
	///for finding controlled train
	var/tram_id

/obj/item/assembly/control/tram/activate()
	if(cooldown)
		return
	var/obj/effect/landmark/tram/to_where = GLOB.assoc_tram_landmarks[to_where_id]
	if(!to_where)
		return
	cooldown = TRUE
	addtimer(VARSET_CALLBACK(src, cooldown, FALSE), 2 SECONDS)
	var/obj/structure/industrial_lift/tram/central/tram_central = GLOB.assoc_trams[tram_id]
	if(!tram_central)
		say("The tram is not responding to call signals. Please send a technician to repair the internals of the tram.")
		return
	if(tram_central.lift_master_datum.travelling) //in use
		say("The tram is already travelling.")
		return
	...
	if(where_we_are == to_where) //already here
	...
	tram_central.lift_master_datum.tram_travel(to_where)
